...
<body>
	...
	<form id="form1">
		<div class="form-group">
			...
			<input id="title" type="text" name="title" class="form-conttrol form-control-sm col-sm-9">
		</div>
		<div class="form-group form-inline">
			<!-- 二级联动 -->
			<label for="title">所属栏目：</label><br>
				<select name="channelId" class="form-control form-control-sm" id="channels">
 				 <option>请选择</option>
			</select>
			<label for="title">所属分类：</label><br>
		...
		<textarea name="content1" cols="100" rows="8" style="width:968.45px;height:280px;visibility:hidden;"><%=htmlspecialchars(htmlData)%></textarea>
		<br />
		<input type="button" onclick="publish()" name="button" class="btn btn-outline-primary" value="发布文章" />
...
$(function(){
	...
	$("#channels").change(function(){
		...
		//根据栏目ID 查询其下分类
		$.get("/channel/selects",{channelId:channelId},function(list){
			 //先清空原有的分类
			 $("#categorys").empty();
			 for(var i in list){
				   //为分类添加option
